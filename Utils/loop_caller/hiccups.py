...
def hiccups(M, cM, B1, B2, IR, chromLen, Diags, cDiags, num, chrom, pw=[2], ww=[5],
    ...
    :param num:
    :param chrom:
    :param pw: List of the peak widths
    :param ww: List of the donut widths
    :param maxww: Maximum donut width
    :param sig:
    :param sumq:
    :param double_fold:
    :param single_fold:
    :param maxapart: Maximum genomic distance
    :param res: resolution
    :param use_raw:
    :param min_marginal_peaks:
    :param onlyanchor:
    :return:
    """
    ...
    p_w = pw_ww_pairs(pw, ww, maxww)  # [(2, 5), (2, 6), ... (2, 20)]
    ...
    for pi, wi in p_w:
        ...
        P1 = set(
            [(i, j) for i in range(wi - pi, ps + wi - pi) for j in range(wi - pi, ps + wi - pi)])  # Center Peak Region
        ...
        for key in Pool_Diags:
            ...
            if (key[0] != wi) and (key[1] != wi) and (key not in P1) and (key not in P2):
                if (not limitCompute) or (limitCompute and bgloc > last_wi) or (
                        limitCompute and bgloc > pi and bgloc <= last_pi):
        ...
        logger.info(
            'Chrom:{0},    ({1},{2}) Total Valid Ratio after This Loop: {3:.3f}'.format(chrom, pi, wi, 1 - left_Ratio))

        if (Valid_Ratio < 0.3) and (wi >= max(ww)):
            logger.info(
                'Chrom:{0},    ({1},{2}) Ratio of valid contact is too small, assign maximum donut width to {3} ...'.format(
                    chrom, pi, wi, wi))
            frozen_w = wi

        if (left_Ratio < 0.03) and (wi >= max(ww)):
            logger.info(
                'Chrom:{0},    ({1},{2}) Very few or no contacts are left, assign maximum donut width to {3} ...'.format(
                    chrom, pi, wi, wi))
            frozen_w = wi

        if wi < frozen_w:
            logger.info('Chrom:{0},    ({1},{2}) {3} Contacts will get into next loop ...'.format(chrom, pi, wi,
                                                                                                  RefIdx[pi].size))
    ...
    for pi, wi in zip(pw, ww):
        ...
        for fl in flocals:
            ...
            for chunk in chunks:
                logger.debug(
                    'Chrom:{0},        ({1},{2}), lv: {3:.3g}, rv: {4:.3g}, Num: {5}'.format(chrom, pi, wi, chunk[0],
                                                                                             chunk[1], chunk[2].size))
                if chunk[2].size > 0:
        ...
        logger.info(
            'Chrom:{0},    Peak width:{1}, Donut width:{2}, Combine two local filters ...'.format(chrom, pi, wi))

        if use_raw:
            preDonuts = dict(
                zip(zip(xpos['K'], ypos['K']), zip(Ovalues['K'], Ovalues['K'], Fold['K'], pvalues['K'], qvalues['K'])))
        else:
            preDonuts = dict(
                zip(zip(xpos['K'], ypos['K']), zip(ICE['K'], Ovalues['K'], Fold['K'], pvalues['K'], qvalues['K'])))
        ...
        for pixel in Donuts:
            ...
            if (donut[2] > double_fold) and (ll[2] > double_fold) and (
                    (donut[2] > single_fold) or (ll[2] > single_fold)):
