...
def training_data_generator(ch_coord, hic_cell_lines, micro_cell_line, epi_names, pos_enc_dim=8,
                            n_epoches=100, batch_size=20, keep_rate=0.1):
    size = 1250
    for ch in ch_coord:
        st, ed = ch_coord[ch]
        assert st % 1000 == 0
    ...
    for ch in ch_coord:
        st, ed = ch_coord[ch]
        print(' Loading HiC strata:', ch, st, ed, keep_rate)
        for i in range(250):
            if i % 25 == 0:
                print(' ', i, '/', 250)
            path = f'/nfs/turbo/umms-drjieliu/usr/temp_Fan/00_HiC_dropout/{keep_rate}'
            strata = None
            for _ in hic_cell_lines:
                s = np.load(f'{path}/{ch}/{ch}_1000bp_strata_{i}.npy')[st // 1000: ed // 1000 - i]
                if strata is None:
                    strata = s
                else:
                    strata += s
...
if __name__ == '__main__':
    ...
    # keep_rates = [0.1, 0.05, 0.025, 0.005]
    keep_rates = [0.1, 0.025, 0.01]
    for keep_rate in keep_rates:
        generator = training_data_generator(ch_coord, hic_cell_lines, micro_cell_line, epi_names,
                                            pos_enc_dim=8, n_epoches=1, batch_size=1, keep_rate=keep_rate)
        ...
        for ch in ch_coord:
            scc(pred_all_strata[ch], micro_all_strata[ch], f'{my_path}/r{keep_rate}_{ch}_pred_micro.txt')
