...
def training_data_generator(ch_coord, hic_cell_lines, micro_cell_line, epi_names, pos_enc_dim=8,
                            n_epoches=100, batch_size=20, loop_epochs=100):
    size = 1250
    for ch in ch_coord:
        st, ed = ch_coord[ch]
        assert st % 1000 == 0
    ...
    #         strata = np.load(f'{path}/HFF/{ch}/{ch}_200bp_strata_{i}.npy')
    ...
    for _epoch in range(n_epoches):
        ...
        for _batch in range(len(idx) // batch_size):
            ...
            yield _epoch, _batch, (hics, epis, pos_encoding, mask), keys
...
if __name__ == '__main__':
    # my_path = os.path.abspath(os.path.dirname(__file__))
    model = CAESAR()
    model.load_weights(f'../surrogateHFF_stripe_temp_model_100.h5')
    # model.load_weights(f'../HFF_stripe_temp_model_100_no_denoise.h5')
    # model.load_weights(f'temp_model_100.h5')

    model_loop = CAESAR_loop()
    model_loop.load_weights(f'../HFF_loop_temp_model_100.h5')

    tissues = ['lung', 'pancreas']

    # ch_coord = {'chr8': (127600000, 127850000), 'chrX': (48600000, 48850000)}
    ch_coord = {'chr2': (85450000, 85700000)}
    epi_names = ['ATAC_seq', 'CTCF', 'H3K4me1', 'H3K4me3', 'H3K27ac', 'H3K27me3']
    for tissue in tissues:
        generator = training_data_generator(ch_coord, ['HFF', 'hESC', 'K562', 'GM12878', 'IMR-90'], tissue, epi_names,
        ...
        for epoch, batch, (hics, epis, pos_enc, mask), kys in generator:
            ...
            _res = _res1 + _res2
            _epi = epis[0, :, :].T
            np.save(f'epi_{tissue}_{kys[0]}.npy', _epi)

            np.save(f'plot_{tissue}_{kys[0]}.npy', _res)

            _res = convolve2d(_res, np.ones((3, 3)) / 9, mode='same')

            visualize_HiC_epigenetics(_res, _epi, f'plot_{tissue}_{kys[0]}.png', colorbar=True,
