...
def training_data_generator(ch_coord, hic_cell_lines, micro_cell_line, epi_names, pos_enc_dim=8,
    ...
    for _epoch in range(n_epoches):
        ...
        for _batch in range(len(idx) // batch_size):
            ...
            yield _epoch, _batch, (hics, epis, pos_encoding, mask), keys, micros
...
if __name__ == '__main__':
    ...
    ch_coord = {'chr16': (57900000, 58150000)}
    ...
    for epoch, batch, (hics, epis, pos_enc, mask), kys, micros in generator:
        print(kys[0])
        ...
        np.save('HFF_epi.npy', _epi)
        np.save('HFF_micro.npy', _micro)
        np.save('HFF_pred.npy', _micro)

        visualize_HiC_epigenetics(_res, _epi, f'HFF_pred.png', colorbar=True,
        ...
        visualize_HiC_epigenetics(_micro, _epi, f'HFF_micro.png', colorbar=True,
                                  interval_after_heatmap=0.,
                                  interval_between_epi=0., epi_labels=epi_names,
                                  epi_yaxis=True, fontsize=20, epi_ratio=0.045,
                                  x_ticks=['', '', '', '', '', ''], vmax=np.quantile(_micro, 0.98)
                                  )
