...
def load_all_loops(cell_line):
    ...
    my_path = os.path.abspath(os.path.dirname(__file__))
    lp_path1 = re.sub('CAESAR_review.*$',
                      f'CAESAR_review/00_utils/micro_loops/{cell_line}_micro_loops_all_1kb/merged_loops.bedpe', my_path)
    lp_path2 = re.sub('CAESAR_review.*$',
                      f'CAESAR_review/00_utils/micro_loops/{cell_line}_micro_loops_all_5kb/merged_loops.bedpe', my_path)
    loops = {}
...
if __name__ == '__main__':
    ...
    for epoch, batch, (hics, epis, pos_enc, mask), kys in generator:
        _res = model.predict([hics, epis, pos_enc, mask])[0, :, :]
        _epi = epis[0, :, :].T
        ky = kys[0]
        pred_all_strata = update_strata(ch_coord, pred_all_strata, ky, _res, correction)

    path = '/nfs/turbo/umms-drjieliu/usr/temp_Fan/07_HFF_loop_all'
    for ch in ch_coord:
