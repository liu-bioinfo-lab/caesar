...
if __name__ == '__main__':
    ...
    # model.load_weights(f'../HFF_stripe_temp_model_100.h5')
    ...
    # ch_coord = {'chr8': (127600000, 127850000)}
    epi_names = ['ATAC_seq', 'CTCF', 'H3K4me1', 'H3K4me3', 'H3K27ac', 'H3K27me3']
    generator = training_data_generator(ch_coord, ['HFF', 'hESC', 'K562', 'GM12878', 'IMR-90'],
                                        'K562', epi_names,
                                        pos_enc_dim=8, n_epoches=1, batch_size=1)
    # generator = training_data_generator(ch_coord, ['HFF'], 'Karpas-422', epi_names,
    #                                     pos_enc_dim=8, n_epoches=1, batch_size=1)
    ...
    for epoch, batch, (hics, epis, pos_enc, mask), kys in generator:
        ...
        np.save(f'4a_example_{batch}.npy', _res)
        _res = convolve2d(_res, np.ones((3, 3)) / 9, mode='same')

        visualize_HiC_epigenetics(_res, _epi, f'4a_example_{batch}.png', colorbar=True,
