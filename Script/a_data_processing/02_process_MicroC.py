...
def micro_counts(cell_lines, raw_path, info_dir):
    ...
    depth.to_csv(f'{info_dir}/micro_depth.csv', sep='\t')


def MicroC_to_strata(cell_lines, info_path, raw_path, processed_path, max_distance=250000, threshold=1.01, rg='hg38'):
    if not os.path.exists(processed_path):
        os.mkdir(processed_path)

    # Count!
    micro_counts(cell_lines, raw_path, info_path)
    ...
    depth = pd.read_csv(f'{info_path}/micro_depth.csv', sep='\t', index_col=0)
    avg_depth = depth.sum(axis=1) / 2  # always use the average
    ...
    for ch in chroms:
        ...
        for cell_line, f in zip(cell_lines, fs):
            ...
            for line in open(f):
                ...
                if p1 > p2:
                    p1, p2 = p2, p1
                if p2 - p1 < n_strata and v > threshold:
